...
namespace ImageProcessor.Data
{
    ...
    {
        public static void SetPixel(BitmapContext context, int x, int y, Color color)
        {
            if (x < 0 || x >= context.Width || y < 0 || y >= context.Height)
                return;

            context.Pixels[y * context.Width + x] = (color.A << 24) | (color.R << 16) | (color.G << 8) | color.B;
        }

        public static void SetPixel(BitmapContext context, int x, int y, byte a, byte r, byte g, byte b)
        {
            if (x < 0 || x >= context.Width || y < 0 || y >= context.Height)
                return;
        ...
        public static void SetPixel(BitmapContext context, int x, int y, byte r, byte g, byte b)
        {
            if (x < 0 || x >= context.Width || y < 0 || y >= context.Height)
                return;
        ...
        public static void SetPixel(BitmapContext context, int i, Color color)
        {
            if (i < 0 || i >= context.Length)
                return;

            context.Pixels[i] = (color.A << 24) | (color.R << 16) | (color.G << 8) | color.B;
        }

        public static void SetPixel(BitmapContext context, int i, byte a, byte r, byte g, byte b)
        {
            if (i < 0 || i >= context.Length)
                return;

            context.Pixels[i] = (a << 24) | (r << 16) | (g << 8) | b;
        }

        public static void SetPixel(BitmapContext context, int i, byte r, byte g, byte b)
        {
            if (i < 0 || i >= context.Length)
                return;

            context.Pixels[i] = (255 << 24) | (r << 16) | (g << 8) | b;
        }
        ...
        public static Color GetPixel(BitmapContext context, int idx)
        {
            if (idx < 0 || idx >= context.Length)
                return Colors.Black;

            var c = context.Pixels[idx];
            var a = (byte)(c >> 24);
