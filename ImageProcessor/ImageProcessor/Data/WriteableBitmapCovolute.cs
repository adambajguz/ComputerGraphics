...
namespace ImageProcessor.Data
{
    public static class WriteableBitmapCovolute
    {
        ...
        #region double convolute
        public static WriteableBitmap Convolute(WriteableBitmap bmp, double[,] kernel)
        {
            double kernelFactorSum = 0;
            foreach (var b in kernel)
            {
                kernelFactorSum += Math.Abs(b);
            }
        ...
        public static WriteableBitmap Convolute(this WriteableBitmap bmp, double[,] kernel, double kernelFactorSum, double kernelOffsetSum)
        {
            var kh = kernel.GetUpperBound(0) + 1;
            var kw = kernel.GetUpperBound(1) + 1;
                        ...
                        for (var x = 0; x < w; x++)
                        {
                            ...
                            for (var kx = -kwh; kx <= kwh; kx++)
                            {
                                ...
                                for (var ky = -khh; ky <= khh; ky++)
                                {
                                    ...
                                    var k = kernel[ky + kwh, kx + khh];
                                    a += ((col >> 24) & 0x000000FF);//* k;
                                    r += (int)(((col >> 16) & 0x000000FF) * k);
                                    g += (int)(((col >> 8) & 0x000000FF) * k);
                                    b += (int)(((col) & 0x000000FF) * k);
                                }
                            }
